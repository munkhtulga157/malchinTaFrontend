...
export default function MaleCow() {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const json = await AsyncStorage.getItem("maleCow");
        if (json) {
          const data = JSON.parse(json);
          setAnimal(data);
        }
        const jsonCount = await AsyncStorage.getItem("maleCowCount");
        if (jsonCount) {
  ...
  const updateDataFromMongoDB = async () => {
    ...
    await AsyncStorage.setItem("maleCow", JSON.stringify(json));
    await AsyncStorage.setItem("maleCowCount", JSON.stringify(jsonCount));
  };
