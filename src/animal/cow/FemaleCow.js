...
export default function FemaleCow() {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const json = await AsyncStorage.getItem("femaleCow");
        if (json) {
          const data = JSON.parse(json);
          setAnimal(data);
        }
        const jsonCount = await AsyncStorage.getItem("femaleCowCount");
        if (jsonCount) {
  ...
  const updateDataFromMongoDB = async () => {
    ...
    await AsyncStorage.setItem("femaleCow", JSON.stringify(json));
    await AsyncStorage.setItem("femaleCowCount", JSON.stringify(jsonCount));
  };
  ...
  const handleRemove = async () => {
    try {
      ...
      await AsyncStorage.setItem("femaleCow", JSON.stringify(updatedAnimal));
