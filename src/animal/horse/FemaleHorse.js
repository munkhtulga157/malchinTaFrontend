...
export default function FemaleHorse() {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const json = await AsyncStorage.getItem("femaleHorse");
        if (json) {
          const data = JSON.parse(json);
          setAnimal(data);
        }
        const jsonCount = await AsyncStorage.getItem("femaleHorseCount");
        if (jsonCount) {
  ...
  const updateDataFromMongoDB = async () => {
    ...
    await AsyncStorage.setItem("femaleHorse", JSON.stringify(json));
    await AsyncStorage.setItem("femaleHorseCount", JSON.stringify(jsonCount));
  };
  ...
  const handleRemove = async () => {
    try {
      ...
      await AsyncStorage.setItem("femaleHorse", JSON.stringify(updatedAnimal));
