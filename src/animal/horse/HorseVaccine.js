...
export default function HorseVaccine({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const json = await AsyncStorage.getItem("hoseVaccine");
        if (json) {
  ...
  const updateDataFromMongoDB = async () => {
    ...
    await AsyncStorage.setItem("horseVaccine", JSON.stringify(json));
  };
  ...
  const handleAddVaccine = () => {
    navigation.navigate("AddHorseVaccine");
  };
