...
import DeleteHerdModal from "../modals/DeleteHerdModal";
...
export default function HorseHerd({ navigation }) {
  ...
  const handleHerdList = async (itemId) => {
    await AsyncStorage.setItem("selectedStallion", JSON.stringify(itemId));
    navigation.navigate("HerdList");
  };

  const handleAddHerd = () => {
    navigation.navigate("ChooseStallion");
  };
      ...
      {loading ? (
        ...
        <View style={styles.container}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                onPress={() => handleHerdList(item._id)}
