...
export default function RemovedHorse() {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const json = await AsyncStorage.getItem("removedHorse");
        if (json) {
          const data = JSON.parse(json);
          setAnimal(data);
        }
        const jsonCount = await AsyncStorage.getItem("removedHorseCount");
        if (jsonCount) {
  ...
  const updateDataFromMongoDB = async () => {
    ...
    await AsyncStorage.setItem("removedHorse", JSON.stringify(json));
    await AsyncStorage.setItem("removedHorseCount", JSON.stringify(jsonCount));
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      await AsyncStorage.setItem("removedHorse", JSON.stringify(updatedAnimal));
            ...
            renderItem={({ item }) => (
              ...
              >
                ...
                <View style={styles.textContainer}>
                  ...
                  <Text style={styles.text}>{`Шалтгаан: ${item.removal}`}</Text>
                </View>
