...
export default function RemovedGoat() {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const json = await AsyncStorage.getItem("removedGoat");
        if (json) {
          const data = JSON.parse(json);
          setAnimal(data);
        }
        const jsonCount = await AsyncStorage.getItem("removedGoatCount");
        if (jsonCount) {
  ...
  const updateDataFromMongoDB = async () => {
    ...
    await AsyncStorage.setItem("removedGoat", JSON.stringify(json));
    await AsyncStorage.setItem("removedGoatCount", JSON.stringify(jsonCount));
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      await AsyncStorage.setItem("removedGoat", JSON.stringify(updatedAnimal));
