...
export default function MaleGoat() {
    ...
    useCallback(() => {
      const fetchData = async () => {
        try {
          ...
          const json = await AsyncStorage.getItem("maleGoat");
          ...
          const jsonCount = await AsyncStorage.getItem("maleGoatCount");
  ...
  const updateDataFromMongoDB = async () => {
    try {
      ...
      await AsyncStorage.setItem("maleGoat", JSON.stringify(json));
      await AsyncStorage.setItem("maleGoatCount", JSON.stringify(jsonCount));
    } catch (error) {
    } finally {
      setLoading(false);
    }
