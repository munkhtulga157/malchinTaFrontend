...
export default function FemaleGoat() {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const json = await AsyncStorage.getItem("femaleGoat");
        if (json) {
          const data = JSON.parse(json);
          setAnimal(data);
        }
        const jsonCount = await AsyncStorage.getItem("femaleGoatCount");
        if (jsonCount) {
  ...
  const updateDataFromMongoDB = async () => {
    ...
    await AsyncStorage.setItem("femaleGoat", JSON.stringify(json));
    await AsyncStorage.setItem("femaleGoatCount", JSON.stringify(jsonCount));
  };
  ...
  const handleRemove = async () => {
    try {
      ...
      await AsyncStorage.setItem("femaleGoat", JSON.stringify(updatedAnimal));
