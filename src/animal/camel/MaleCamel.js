...
export default function MaleCamel() {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const json = await AsyncStorage.getItem("maleCamel");
        if (json) {
          const data = JSON.parse(json);
          setAnimal(data);
        }
        const jsonCount = await AsyncStorage.getItem("maleCamelCount");
        if (jsonCount) {
  ...
  const updateDataFromMongoDB = async () => {
    ...
    await AsyncStorage.setItem("maleCamel", JSON.stringify(json));
    await AsyncStorage.setItem("maleCamelCount", JSON.stringify(jsonCount));
  };
  ...
  const handleRemove = async () => {
    try {
      ...
      await AsyncStorage.setItem("maleCamel", JSON.stringify(updatedAnimal));
